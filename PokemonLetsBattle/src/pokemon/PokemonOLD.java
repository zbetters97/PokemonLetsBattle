...
public class PokemonOLD {
	...
	/** CONSTRUCTORS **/
	public PokemonOLD(PokedexOLD p, int lvl, Entity ball) {	
	...
	public PokemonOLD(PokemonOLD old, PokedexOLD p) {	
		// STAT FORMULA REFERENCE: https://pokemon.fandom.com/wiki/Statistics
	...
	public PokemonOLD(PokedexOLD p, Entity ball, char sex, int level, int bxp, int xp, int nxp,
	...
	public static PokemonOLD evolve(PokemonOLD oldPokemon) {
		
		PokemonOLD evolvedForm = null;
		
		for (PokedexOLD base : PokedexOLD.values()) {
			if (base.getIndex() == oldPokemon.getEvolveIndex()) {
				evolvedForm = new PokemonOLD(oldPokemon, base);				
				break;
			}
		}		
	...
	/** MOVE METHODS **/
	private static void mapMoves(PokemonOLD p) {
		
		// if pokemon not already mapped to moveset
        if (p.getMoveSet().isEmpty()) {
        	
        	// add each move to passed in pokemon object
        	if (PokedexOLD.getMovemap().get(p.pokemon).size() > 0) {
        		for (int i = 0; i < PokedexOLD.getMovemap().get(p.pokemon).size(); i++) {        	
                	p.learnMove(PokedexOLD.getMovemap().get(p.pokemon).get(i));
                } 	
        	}            	
        }
	}
	...
	/** GET POKEMON METHODS **/	
	public static PokemonOLD getPokemon(PokedexOLD poke, int level, Entity capturedBall) {
		
		PokemonOLD pokemon = null;
		
		for (PokedexOLD p : PokedexOLD.values()) {
			if (p == poke) {
				pokemon = new PokemonOLD(p, level, capturedBall);
				break;
			}
		}
	...
	public static PokemonOLD getPokemonByIndex(int index, int level, Entity capturedBall) {
	...
	public PokedexOLD getPokemon() { return pokemon; }
